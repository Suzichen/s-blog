...
function getSiteConfig(): SiteConfig {
  ...
  const tzMatch = configContent.match(
    /(?:timezone|'timezone'|"timezone")\s*:\s*["']([^"']+)["']/,
  );
  return {
    timezone: tzMatch?.[1],
  };
...
function getSummary(content: string, length: number = 140): string {
  ...
  return (
    plainText.substring(0, length) + (plainText.length > length ? "..." : "")
  );
}
...
function main() {
  ...
  for (const file of files) {
    ...
    // Format date — gray-matter parses YAML dates as UTC Date objects.
    // Store without 'Z' so the frontend treats it as local time.
    let dateStr = "";
    if (data.date) {
      try {
        ...
        if (siteConfig.timezone && hasTimezone) {
          try {
            ...
            console.warn(
              `Invalid timezone configuration: ${siteConfig.timezone}. Falling back to default behavior.`,
            );
            dateStr = `${d.getUTCFullYear()}-${p(d.getUTCMonth() + 1)}-${p(d.getUTCDate())}T${p(d.getUTCHours())}:${p(d.getUTCMinutes())}:${p(d.getUTCSeconds())}`;
          }
        } else {
          // If no timezone was specified in YAML, gray-matter parsed it as UTC.
          // We use getUTC* to extract the exact local time the user wrote.
          dateStr = `${d.getUTCFullYear()}-${p(d.getUTCMonth() + 1)}-${p(d.getUTCDate())}T${p(d.getUTCHours())}:${p(d.getUTCMinutes())}:${p(d.getUTCSeconds())}`;
        }
      } catch (e) {
    ...
    const summary =
      data.preview || data.description || data.excerpt || getSummary(content);
